<template>
  <div>
    <div
      class="ln-hero-banner ln-hero-banner--h550 ln-hero-banner--theme-default ln-hero-banner--valign-middle ln-hero-banner--align-right"
    >
      <div class="container ln-hero-banner__container-wrapper lnet-slider--transition">
        <div class="ln-hero-banner__element-wrapper">
          <span
            class="ln-headline--hero ln-hero-banner__headline"
          >FULLY INTEGRATED DATA</span>
        </div>
      </div>
      <div
        class="ln-hero-banner__background-image"
        style="background-image: url('https://www.lucanet.com/media/cache/slider_300_lg/images/slider-image/header-loesungen-datenmanagement-1920-300.jpg')"
      ></div>
    </div>
    <main>
      <div class="ln-solution">
        <div class="full-width-block full-width-block--large-space-above ln-solution__section">
          <div class="container">
            <div class="row">
              <div class="ln-solution-text col-xs-12 col-sm-6 mb10">
                <h1
                  class="ln-headline--large ln-solution-text__headline"
                >The software for data collection and validation</h1>
                <span class="ln-solution-text__text">
                  <p>The lack of quality in financial data, which is usually drawn from a range of different system landscapes and source systems, represents a huge challenge for many finance departments.</p>
                  <p>Say goodbye to financial data that needs to be painstakingly incorporated into reports and integrated into plans and consolidated financial statements without first being validated. LucaNet offers a software tool that will make your everyday routine noticeably easier with simple data collection and reliable data validation.</p>
                </span>
              </div>
              <div class="col-xs-12 col-sm-6 ln-solution-video">
                <figure>
                  <div class="embed-responsive embed-responsive-16by9">
                    <video width="320" height="240" controls>
                      <source
                        src="https://www.youtube-nocookie.com/embed/eyNYfRmbQPc?rel=0&amp;enablejsapi=1&amp;origin=https%3A%2F%2Fwww.lucanet.com"
                        type="video/mp4"
                      />
                    </video>
                  </div>
                </figure>
              </div>
            </div>
          </div>
        </div>
        <div class="full-width-block ln-solution__section full-width-block--large-space-above">
          <div class="container">
            <div class="row">
              <div class="col-xs-12">
                <h2
                  class="ln-headline--normal ln-solution__section-headline tal"
                >The path to maximum data quality</h2>
              </div>
            </div>
            <div class="row ln-solution-text mt35">
              <div class="col-xs-12">
                <div class="ln-solution-text__text">
                  <p>Our reporting software is sure to be the right tool for you thanks to all the functions it offers, which include:</p>
                  <ul style="list-style-type: square;font-size:18px;padding-left:30px">
                    <li>Automated validation options</li><li>Workflow support via integrated task management</li><li>Transparency about the state of data collection</li><li>Secure and error-free data transfer</li><li>Data files that can be audited</li></ul>
                  <p style="text-align:center">
                    <br />Do you require assistance in the hunt for the right reporting software? Then be sure to include our catalog in your selection process. You can download it free of charge.
                  </p>
                  <p style="text-align:center">
                    <router-link
                      class="btn btn-primary"
                      to="https://www.lucanet.com/en/info-center/brochure/financial-consolidation"
                      target="_self"
                    >Download our brochure</router-link>
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div
          class="full-width-block ln-solution__section ln-metastream ln-metastream--five ln-metastream--middle full-width-block--large-space-above full-width-block--large-space-below"
        >
          <div class="container">
            <div class="row">
              <div class="col-xs-12">
                <h2
                  class="ln-headline--normal ln-solution__section-headline tac"
                >An intelligent solution for management reporting: How LucaNet can benefit you</h2>
              </div>
            </div>
            <div class="row mt35">
              <div class="ln-teaser ln-teaser--grid-3">
                <div class="ln-teaser__grid-item">
                  <div class="ln-teaser__item ln-teaser__item--benefit">
                    <span class="ln-teaser__body">
                      <span class="ln-teaser__headline">Comprehensive view</span>
                      <span class="ln-teaser__text">
                        <p
                          style="text-align:center"
                        >Tailored to your specific needs, LucaNet provides standard and ad-hoc reports for all the components of a professional reporting system. Features such as comparative views, time series analyses, and exchange rate scenarios are included and are illustrated using graphics and spark lines.</p>
                      </span>
                    </span>
                    <img src="../../assets/icon-f.png" class="ln-teaser__badge" />
                  </div>
                </div>
                <div class="ln-teaser__grid-item">
                  <div class="ln-teaser__item ln-teaser__item--benefit">
                    <span class="ln-teaser__body">
                      <span class="ln-teaser__headline">Flexibility</span>
                      <span class="ln-teaser__text">
                        <p
                          style="text-align:center"
                        >This LucaNet tool makes it possible for you to make changes to structures in a way that is flexible and straightforward. It also presents important data in a transparent manner and enables you to add KPIs and statistical information – all within a single, convenient system.</p>
                        <p style="text-align:center">
                          <a
                            href="https://www.lucanet.com/en/technology/software-architecture#source-systems"
                          >Learn more</a>
                        </p>
                      </span>
                    </span>
                    <img src="../../assets/icon-f.png" class="ln-teaser__badge" />
                  </div>
                </div>
                <div class="ln-teaser__grid-item">
                  <div class="ln-teaser__item ln-teaser__item--benefit">
                    <span class="ln-teaser__body">
                      <span class="ln-teaser__headline">Excel integration</span>
                      <span class="ln-teaser__text">
                        <p
                          style="text-align:center"
                        >LucaNet offers full Excel integration for planning and reporting purposes. Use LucaNe<strong>.Excel-Add-In</strong> for interactive analysis and processing of data and LucaNet.<strong>Excel-Reporting</strong> to generate standard reports.</p>
                      </span>
                    </span>
                    <img src="../../assets/icon-f.png" class="ln-teaser__badge" />
                  </div>
                </div>
                <div class="ln-teaser__grid-item">
                  <div class="ln-teaser__item ln-teaser__item--benefit">
                    <span class="ln-teaser__body">
                      <span class="ln-teaser__headline">BI integration</span>
                      <span class="ln-teaser__text">
                        <p
                          style="text-align:center"
                        >LucaNet supports connections to other business analytics systems. Whether you’re using a BI tool such as QlikView, Tableau, or Microsoft Power BI, you can easily use the relevant data from LucaNet to present it in the BI tool of your choice.</p>
                      </span>
                    </span>
                    <img src="../../assets/icon-f.png" class="ln-teaser__badge" />
                  </div>
                </div>
                <div class="ln-teaser__grid-item">
                  <div class="ln-teaser__item ln-teaser__item--benefit">
                    <span class="ln-teaser__body">
                      <span class="ln-teaser__headline">Time savings</span>
                      <span class="ln-teaser__text">
                        <p
                          style="text-align:center"
                        >With reporting software from LucaNet, you’ll reduce the manual effort you put into data collection and validation. You’ll finally be able to focus on the things that matter, like analyzing the data. You’ll become a respected business partner within your company.</p>
                      </span>
                    </span>
                    <img src="../../assets/icon-f.png" class="ln-teaser__badge" />
                  </div>
                </div>
                <div class="ln-teaser__grid-item">
                  <div class="ln-teaser__item ln-teaser__item--benefit">
                    <span class="ln-teaser__body">
                      <span class="ln-teaser__headline">Self-service solution</span>
                      <span class="ln-teaser__text">
                        <p
                          style="text-align:center"
                        >With LucaNet, the secret lies in self-service technology. As a professional user, you can administer the system on your own and make any adjustments independently within the tool. That will also please your IT department, as it will have a lighter support workload.</p>
                      </span>
                    </span>
                    <img src="../../assets/icon-f.png" class="ln-teaser__badge" />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="full-width-block ln-solution__section ln-solution__section--lg-up">
          <div class="container">
            <div class="row">
              <div class="col-xs-12">
                <h2
                  class="ln-headline--normal ln-solution__section-headline tac"
                >Five surefire steps to implementing management reporting:</h2>
              </div>
            </div>
            <div class="row mt35" id="solution-processes">
              <div class="col-md-4 ln-solution-process">
                <i class="ln-solution-process__connector-horizontal"></i>
                <i class="ln-solution-process__connector-vertical"></i>
                <div class="ln-solution-process__body">
                  <div class="ln-solution-process__icon">
                    <img src="../../assets/icon-p1.png" />
                    <span class="ln-solution-process__badge">1</span>
                  </div>
                  <span class="ln-solution-process__headline">Definition of the data model</span>
                </div>
                <div class="dropdown">
                  <img
                    src="../../assets/icon-togger.png"
                    class="dropdownMenu"
                    id="dropdownMenu2"
                    data-toggle="dropdown"
                    aria-haspopup="true"
                    aria-expanded="false"
                  />
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>Define your planning elements (locations, cost centers, projects, customers, etc.) with just a few clicks.</li>
                    <li>Entering a planning period is also very easy.</li>
                  </ul>
                </div>
              </div>
              <div class="col-md-4 ln-solution-process">
                <img src="../../assets/icon-left.png" style="position: absolute;top:45%;left:0" />
                <div class="ln-solution-process__body">
                  <div class="ln-solution-process__icon">
                    <img src="../../assets/icon-p2.png" />
                    <span class="ln-solution-process__badge">2</span>
                  </div>
                  <span class="ln-solution-process__headline">Transfer of actual data</span>
                </div>
                <div class="dropdown">
                  <img
                    src="../../assets/icon-togger.png"
                    class="dropdownMenu"
                    id="dropdownMenu2"
                    data-toggle="dropdown"
                    aria-haspopup="true"
                    aria-expanded="false"
                  />
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>LucaNet makes importing your actual data from the relevant source systems a snap.</li>
                  </ul>
                </div>
              </div>
              <div class="col-md-4 ln-solution-process">
                <img
                  src="../../assets/icon-left3.png"
                  style="position: absolute;bottom:-55px;left:48%"
                />
                <div class="ln-solution-process__body">
                  <div class="ln-solution-process__icon">
                    <img src="../../assets/icon-p3.png" />
                    <span class="ln-solution-process__badge">3</span>
                  </div>
                  <span class="ln-solution-process__headline">Setup of sub-plans</span>
                </div>
                <div class="dropdown">
                  <img
                    src="../../assets/icon-togger.png"
                    class="dropdownMenu"
                    id="dropdownMenu2"
                    data-toggle="dropdown"
                    aria-haspopup="true"
                    aria-expanded="false"
                  />
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>Put together sub-plans that cover sales, expenses, investments, borrowing, and more.</li>
                    <li>In doing so, you can use predefined templates for mapping your sub-plans.</li>
                  </ul>
                </div>
              </div>
              <div class="col-md-4 ln-solution-process">
                <img
                  src="../../assets/icon-left3.png"
                  style="position: absolute;bottom:-55px;left:48%"
                />
                <div class="ln-solution-process__body">
                  <div class="ln-solution-process__icon">
                    <img src="../../assets/icon-c6.png" />
                    <span class="ln-solution-process__badge">6</span>
                  </div>
                  <span class="ln-solution-process__headline">Accounting principles</span>
                </div>
                <div class="dropdown">
                  <img
                    src="../../assets/icon-togger.png"
                    class="dropdownMenu"
                    id="dropdownMenu2"
                    data-toggle="dropdown"
                    aria-haspopup="true"
                    aria-expanded="false"
                  />
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>Posting focus means straightforward collection of reconciliation postings (HB II, IFRS, US GAAP etc.)</li>
                    <li>Simultaneous presentation of P&L and balance sheet structures</li>
                    <li>System-supported validation of all postings generated</li>
                    <li>Seamless documentation by means of change log</li>
                  </ul>
                </div>
              </div>
              <div class="col-md-4 ln-solution-process">
                <img src="../../assets/icon-left2.png" style="position: absolute;top:45%;left:0" />
                <div class="ln-solution-process__body">
                  <div class="ln-solution-process__icon">
                    <img src="../../assets/icon-p5.png" />
                    <span class="ln-solution-process__badge">5</span>
                  </div>
                  <span class="ln-solution-process__headline">Integration of sub-plans</span>
                </div>
                <div class="dropdown">
                  <img
                    src="../../assets/icon-togger.png"
                    class="dropdownMenu"
                    id="dropdownMenu2"
                    data-toggle="dropdown"
                    aria-haspopup="true"
                    aria-expanded="false"
                  />
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>Aggregate your planning data from the individual plans into one transparent overall plan.</li>
                  </ul>
                </div>
              </div>
              <div class="col-md-4 ln-solution-process">
                <img src="../../assets/icon-left2.png" style="position: absolute;top:45%;left:0" />
                <div class="ln-solution-process__body">
                  <div class="ln-solution-process__icon">
                    <img src="../../assets/icon-p4.png" />
                    <span class="ln-solution-process__badge">4</span>
                  </div>
                  <span class="ln-solution-process__headline">Implementation of planning</span>
                </div>
                <div class="dropdown">
                  <img
                    src="../../assets/icon-togger.png"
                    class="dropdownMenu"
                    id="dropdownMenu2"
                    data-toggle="dropdown"
                    aria-haspopup="true"
                    aria-expanded="false"
                  />
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>To finalize your planning, start by entering your centralized or decentralized planning data.</li>
                  </ul>
                </div>
              </div>
              <div class="col-md-4 ln-solution-process">
                <!-- <img src="../../assets/icon-left.png" style="position: absolute;top:45%;left:0"/> -->
                <div class="ln-solution-process__body">
                  <div class="ln-solution-process__icon">
                    <img src="../../assets/icon-c7.png" />
                    <span class="ln-solution-process__badge">7</span>
                  </div>
                  <span class="ln-solution-process__headline">Consolidation run</span>
                </div>
                <div class="dropdown">
                  <img
                    src="../../assets/icon-togger.png"
                    class="dropdownMenu"
                    id="dropdownMenu2"
                    data-toggle="dropdown"
                    aria-haspopup="true"
                    aria-expanded="false"
                  />
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>
                      Fully automated consolidation that covers
                      <ul style="list-style-type:circle;font-size:16px;padding-left:30px">
                        <li>all the steps in consolidation (equity elimination; elimination of intercompany debt; elimination of intercompany income and expense; elimination of intercompany profit and loss)</li>
                        <li>and group structures of any depth (legal consolidation, by segment consolidation, etc.)</li>
                      </ul>
                    </li>
                  </ul>
                </div>
              </div>
              <div class="col-md-4 ln-solution-process">
                <!-- <img src="../../assets/icon-left.png" style="position: absolute;top:45%;left:0" /> -->
                <div class="ln-solution-process__body">
                  <div class="ln-solution-process__icon">
                    <img src="../../assets/icon-c8.png" />
                    <span class="ln-solution-process__badge">8</span>
                  </div>
                  <span class="ln-solution-process__headline">Quality assurance</span>
                </div>
                <div class="dropdown">
                  <img
                    src="../../assets/icon-togger.png"
                    class="dropdownMenu"
                    id="dropdownMenu2"
                    data-toggle="dropdown"
                    aria-haspopup="true"
                    aria-expanded="false"
                  />
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                    <li>
                      Fully automated consolidation that covers
                      <ul style="list-style-type:circle;font-size:16px;padding-left:30px">
                        <li>System-supported status reports for quality assurance</li>
                      </ul>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
        <!--  -->
        <div class="full-width-block ln-solution__section">
          <div class="container">
            <div class="mb35">
              <div class="row mb35">
                <div class="col-xs-12">
                  <h2
                    class="ln-headline--normal ln-solution__section-headline tal"
                  >Only LucaNet is this simple</h2>
                </div>
              </div>
              <div class="row m0">
                <div class="col-xs-12 col-md-4 p0">
                  <div class="ln-tab-list ln-tab-list--hide-mobile">
                    <div
                      class="ln-tab-list__tab ln-tab-list__tab--padding-small"
                      v-for="(item, index) in list"
                      :key="index"
                      @click="onList(index)"
                      :style=" curidenx == index ? 'background: #04a88c;' : '' "
                    >
                      <span class="ln-tab-list__headline">{{ item }}</span>
                    </div>
                  </div>
                </div>
                <div class="col-xs-12 col-md-8 p0">
                  <div class="ln-screenshot" style="padding: 70px 100px;background: #f2f2f2;">
                    <div class="ln-media-frame ln-media-frame--laptop" style="position: relative;">
                      <img src="/img/aa-img.e5bec5b9.jpg" style="position: absolute; top: 16px; width: 76%; left: 12%;">
                    </div>
                    <div class="ln-screenshot__info">
                      <p>New rules can easily be created by entering conditions (including as if-then rules) and then rolled out immediately to all reporting units.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <!--  -->
        <div class="full-width-block ln-solution__section">
          <div class="container">
            <div class="row ln-solution-text mt35">
              <div class="col-xs-12">
                <div class="ln-solution-text__text">
                  <h3
                    style="text-align: center;margin:32px 0 16px;font-size:30px"
                  >Learn even more about our tool for data validation</h3>
                  <p
                    style="text-align: center;"
                  >Interested in seeing our software in action? We’d be pleased to offer you a live, personalized demo with no obligation – either at your offices or in an online presentation!</p>
                  <p style="text-align:center">
                    <a
                      class="btn btn-raspberry"
                      href="https://www.lucanet.com/en/live-demo/"
                      target="_top"
                    >Schedule a live demo</a>
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <!--  -->
        <div class="full-width-block ln-solution__section full-width-block--large-space-below">
          <div class="container">
            <div class="row">
              <div class="col-xs-12">
                <h2 class="ln-headline--normal ln-solution__section-headline  tac">More on Data Collection and Validation</h2>
              </div>
            </div>
            <div class="row ln-solution-text mt35">
              <div class="col-xs-12">
                <div class="ln-solution-text__text">
                  <div class="ln-teaser ln-teaser--center"> 
                    <div class="ln-teaser__grid-item" style="flex: 0 0 25%; max-width: 25%;">
                      <router-link to="" class="ln-teaser__item ln-teaser__item--scale">
                        <img src="../../assets/whitepapers-e-books.jpg" style="width:100%"/>
                        <span class="ln-teaser__footer ln-teaser__footer--block"><span class="btn btn-green" style="margin:0;width:100%">White Paper</span></span>
                      </router-link>
                    </div>
                    <div class="ln-teaser__grid-item" style="flex: 0 0 25%; max-width: 25%;">
                      <router-link to="" class="ln-teaser__item ln-teaser__item--scale">
                        <img src="../../assets/case-study-man.jpg" style="width:100%"/>
                        <span class="ln-teaser__footer ln-teaser__footer--block"><span class="btn btn-green" style="margin:0;width:100%">Success Story Basic-Fit</span></span>
                      </router-link>
                    </div>
                    <div class="ln-teaser__grid-item" style="flex: 0 0 25%; max-width: 25%;">
                      <router-link to="" class="ln-teaser__item ln-teaser__item--scale">
                        <img src="../../assets/brochures.jpg" style="width:100%"/>
                        <span class="ln-teaser__footer ln-teaser__footer--block"><span class="btn btn-green" style="margin:0;width:100%">Brochure</span></span>
                      </router-link>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </main>
  </div>
</template>

<script>
export default {
  data() {
    return {
      curidenx: 0,
      list: ['Status monitor for all reporting units', 'Integrated validation rules', 'Simple creation of new validation rules']
    }
  },
  methods: {
    onList(index) {
      this.curidenx = index
    },
    onCheangeInput() {
      
    }
  }
}
</script>

<style lang="less" scoped>
.ln-media-frame--laptop {
    padding-top: 60.7536764706%;
    background: transparent url('../../assets/frame_laptop.png') no-repeat;
    background-size: contain;
}
.ln-teaser__item--scale:hover{
  transform: scale(1.05);
}
.col-md-8 {
  width: 66.66667%;
}
.ln-tab-list__headline {
  display: block;
  font-weight: 300;
  font-size: 18px;
}
.ln-tab-list__tab {
  display: block;
  position: relative;
  padding: 0.625rem 2.8125rem;
  border-bottom: 1px solid #eaeaea;
  background: #fff;
  cursor: pointer;
}
.ln-tab-list__tab--padding-small {
  padding: 10px 20px;
}
.ln-tab-list {
  display: flex;
  flex-direction: column;
  flex-wrap: nowrap;
}
.mb35 {
  margin-bottom: 35px !important;
}
.dropdownMenu {
  position: absolute;
  right: -10px;
  top: -15px;
  z-index: 9;
}
.dropdown-menu {
  padding: 20px;
  width: 98%;
  margin: 0;
  border-top: none;
  border-bottom: 1px solid #202a60;
  border-left: 1px solid #202a60;
  border-right: 1px solid #202a60;
  box-shadow: none;
  border-radius: 0;
  z-index: 1;
  list-style-type: square;
  padding-left: 50px;
}
.ln-solution-process__body {
  padding-top: 20px;
  padding-bottom: 10px;
  text-align: center;
  border-bottom: 1px solid #202a60;
  cursor: pointer;
}
.ln-solution-process__icon {
  display: inline-block;
  position: relative;
}

ul li {
  font-family: none;
}
.mt35 {
  margin-top: 35px !important;
}
.pos-rel {
  position: relative !important;
}
.ln-hero-banner {
  width: 100%;
  height: 300px;
  position: relative;
  z-index: 1;
  background-position: top;
  background-size: cover;
  background-repeat: no-repeat;
  overflow: hidden;
}
.ln-hero-banner--align-left,
.ln-hero-banner--align-right {
  text-align: center;
}
.ln-hero-banner__container-wrapper {
  position: relative;
  top: 50%;
  transform: translateY(-50%);
  z-index: 4;
}
.ln-hero-banner__headline,
.ln-hero-banner__subline {
  display: inline-block;
  color: #fff;
}
.ln-hero-banner__background-image {
  display: block;
  position: absolute;
  top: 0;
  width: 100%;
  height: 100%;
  z-index: 2;
  background-position: 50%;
  background-repeat: no-repeat;
}
.ln-headline--hero,
.ln-headline--teaser--small-mb {
  margin-bottom: 16px;
}
.ln-headline--hero {
  font-family: sans-serif;
  font-style: normal;
  font-weight: 400;
  line-height: 1.2;
  margin-top: 0;
  font-size: 48px;
  line-height: 1.1;
  font-weight: 300;
  text-transform: uppercase;
}
.btn {
  display: inline-block;
  margin-top: 1.25rem;
  padding: 10px 1.5625rem;
  vertical-align: middle;
  border-radius: 0;
  border: 2px solid #04a88c;
  color: #fff;
  background-color: #04a88c;
  box-shadow: none;
  font-size: 0.8125rem;
  font-weight: 400;
  line-height: 1.5;
  text-align: center;
  text-transform: uppercase;
  text-decoration: none;
  cursor: pointer;
  -webkit-user-select: none;
  -moz-user-select: none;
  -ms-user-select: none;
  user-select: none;
  outline: 0;
  transition: color 0.15s ease-in-out, background-color 0.15s ease-in-out,
    border-color 0.15s ease-in-out, box-shadow 0.15s ease-in-out;
}
.btn {
  white-space: normal;
  font-size: 16px;
}
.btn-white {
  color: #202a60;
  background-color: #fff;
  border: 2px solid #fff;
  text-decoration: none;
  &:hover {
    background: #202a60;
    color: #fff;
  }
}
.ln-hero-banner__cta {
  display: inline-block;
  /* padding: .625rem 1rem;  */
  padding: 10px 14px;
  margin: 1.875rem 10px 0 0;
}
.ln-hero-banner__cta:first-of-type {
  margin-left: 0;
}
.ln-solution,
.ln-solution__section {
  position: relative;
}
.mb10 {
  margin-bottom: 10px !important;
}
.ln-solution-text__headline {
  display: block;
  margin-top: 0;
  margin-bottom: 20px;
}
.ln-solution-text__text {
  margin: 0 0 10px;
}
.ln-solution-demo-teaser__headline,
.ln-solution__section-headline {
  display: block;
  margin-top: 0;
}
.ln-metastream {
  background-repeat: no-repeat;
  background-size: 100% auto;
}
.ln-metastream--middle {
  background-position: 50%;
}
.full-width-block--large-space-below {
  padding: 65px 0;
}
.ln-teaser--grid-3 {
  justify-content: center;
}
.ln-teaser {
  display: flex;
  flex-wrap: wrap;
  align-items: stretch;
}
.ln-teaser__grid-item {
  position: relative;
  flex: 0 0 100%;
  max-width: 100%;
  padding-left: 15px;
  padding-right: 15px;
  margin-bottom: 20px;
  outline: 0 !important;
}
.ln-teaser__item {
  position: relative;
  display: flex;
  flex-direction: column;
  align-items: stretch;
  width: 100%;
  height: 100%;
  margin: 0;
  border: 0;
  color: #333;
  background-color: #fff;
  box-shadow: 2px 2px 36px 20px rgba(0, 0, 0, 0.03);
  cursor: pointer;
  transition: box-shadow 0.5s ease, transform 0.5s ease;
  outline: 0 !important;
}
.ln-teaser__item--benefit {
  cursor: default;
  text-align: center;
}
.ln-teaser__body {
  display: flex;
  flex-direction: column;
  padding: 30px 35px;
}
.ln-teaser__item--benefit .ln-teaser__headline {
  font-size: 22px;
  margin-bottom: 20px;
}
.ln-teaser__headline {
  display: inline-block;
  color: #202a60;
  margin-bottom: 0.625rem;
  font-size: 1.25rem;
  font-weight: 400;
  line-height: 26px;
}
.ln-teaser__text:last-child {
  margin: 0;
}
.ln-teaser__text {
  font-size: 1rem;
  font-weight: 300;
  line-height: 1.5;
  color: #333;
}
.ln-teaser__badge {
  display: block;
  width: 32px;
  height: 32px;
  position: absolute;
  right: -0.75rem;
  top: -0.75rem;
  border-radius: 50%;
  color: #fff;
}
.ln-solution-process {
  position: relative;
  height: 123px;
  margin-bottom: 70px;
  padding-left: 50px;
  padding-right: 50px;
}
.ln-solution-process__connector-horizontal,
.ln-solution-process__connector-vertical {
  display: none;
  height: 17px;
  width: 17px;
  font: normal normal normal 34px/1 Open Sans, sans-serif;
  text-rendering: auto;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
  color: #b80e80;
}
.ln-solution-process__badge {
  display: inline-block;
  position: absolute;
  left: -50%;
  top: 0;
  text-align: center;
  background-color: #04a88c;
  color: #fff;
  font-weight: 300;
  font-style: normal;
  font-size: 14px;
  line-height: 22px;
  letter-spacing: 0.025em;
  border-radius: 11px;
  width: 22px;
  height: 22px;
}
.ln-solution-process__headline {
  margin-top: 20px;
  display: block;
  color: #202a60;
  font-weight: 400;
  font-style: normal;
  font-size: 16px;
  line-height: 20px;
  // margin-top: 0;
  margin-bottom: 0;
  letter-spacing: 0.025em;
}
.ln-screenshot {
  display: block;
  width: 100%;
  position: relative;
  padding: 70px 100px;
  background: #f2f2f2;
  max-height: 863px;
}
.ln-screenshot__info {
  display: block;
  width: 100%;
  margin: 32px 0 16px;
}
.btn-raspberry {
    color: #fff;
    background-color: #b80e80;
    border: 2px solid #b80e80;
    text-decoration: none;
}




@media (min-width: 62rem) {
  p {
    font-size: 18px;
  }
  .ln-hero-banner--align-right {
    text-align: right;
  }
  .ln-hero-banner--h550 {
    height: 300px;
  }

  .ln-hero-banner--valign-middle .ln-hero-banner__container-wrapper {
    top: 50%;
    transform: translateY(-50%);
  }
  .ln-hero-banner--align-right .ln-hero-banner__element-wrapper {
    margin-left: auto;
    margin-right: 0;
  }
  .ln-hero-banner__element-wrapper {
    width: 66%;
  }
  .ln-hero-banner__headline,
  .ln-hero-banner__subline {
    padding: 10px;
    background-color: hsla(0, 0%, 100%, 0.8);
  }
  .ln-hero-banner__headline {
    letter-spacing: 0;
    color: #008bd2;
  }
  .ln-hero-banner__subline {
    color: #333;
    margin-bottom: 10px;
  }
  .ln-hero-banner__cta {
    margin: 30px 10px 0 0;
  }
  .ln-customer-alert__wrapper {
    flex-direction: row;
    justify-content: space-around;
  }
  .full-width-block--zero-space-below {
    padding-bottom: 0;
  }
  .full-width-block--large-space-above {
    padding-top: 65px;
  }
  .ln-headline,
  .ln-headline--large {
    font-family: Roboto, Helvetica Neue, Arial, sans-serif;
    font-style: normal;
    font-weight: 400;
    line-height: 1.2;
    margin-top: 0;
    margin-bottom: 20px;
  }
  .ln-headline--large {
    font-size: 43px;
    color: #202a60;
  }
  .full-width-block {
    padding: 32px 0;
  }
  .full-width-block--large-space-above {
    padding-top: 60px;
  }
  .full-width-block--large-space-below {
    padding-bottom: 65px;
  }
  .full-width-block--left-half > .container > .row,
  .full-width-block--right-half > .container > .row {
    max-width: 50%;
    flex-basis: 50%;
  }
  .ln-bluebox--small {
    margin-bottom: 100px;
    padding-bottom: 0;
  }
  .ln-teaser__icon {
    width: 140px;
    height: 140px;
  }
  .ln-teaser__icon > i {
    height: 66px;
    font-size: 70px;
  }
  .ln-teaser__text {
    font-size: 18px;
  }
  .ln-teaser__item:hover {
    text-decoration: none;
  }
  .ln-teaser__headline {
    font-size: 22px;
  }
  .ln-bluebox--top:before {
    top: -6.25rem;
  }
  .ln-bluebox--narrow:before {
    width: 43%;
  }
  .ln-bluebox:before {
    display: block;
  }
  .ln-headline--normal {
    font-size: 38px;
  }
  .col-md-4 {
    width: 33.33333%;
  }
  .top-statement-box .image-block {
    width: 120px;
    height: 120px;
  }
  .top-statement-box .statement-position {
    min-height: 50px;
  }
  .col-sm-6 {
    width: 50%;
  }
  .ln-metastream--five {
    background-image: url("../../assets/meta_five_xl.png");
  }
  .ln-solution__section--lg-up {
    display: block;
  }
  .col-md-4 {
    width: 33.33333%;
  }
}

@media screen and (min-width: 1200px) {
  .btn {
    padding: 12px 12px;
    margin: 40px 10px 0 0;
    font-size: 16px;
  }
  .btn--small-margin {
    margin: 4px 12px 12px 0;
  }
  .container {
    width: 1160px;
    max-width: 1160px;
  }
  .ln-headline--hero {
    font-size: 44px;
  }
  .ln-hero-banner__subline {
    font-size: 28px;
    font-weight: 300;
    line-height: 1.1;
  }
  .ln-teaser .ln-teaser__grid-item {
    flex: 0 0 33%;
    max-width: 33%;
  }
  .ln-metastream--four {
    background-image: url("../../assets/meta_four_xl.png");
  }
}
@media screen and (min-width: 1500px) {
  .container {
    width: 1400px;
    max-width: 1400px;
  }
  .ln-headline--hero {
    font-size: 53px;
  }
  .ln-hero-banner__subline {
    font-size: 32px;
  }
}
</style>